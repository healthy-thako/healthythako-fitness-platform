...
// CSS fallback gradient component
const CSSFallback: React.FC<{ colorStops: string[] }> = ({ colorStops }) => {
  const keyframes = `
    @keyframes aurora-background-gradient {
    ...
    <>
      ...
      <div 
        ...
        style={{
          ...
          animation: 'aurora-background-gradient 8s ease infinite'
        }}
      />
    </>
  );
...
export default function AuroraBackground(props: AuroraProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!renderer || !renderer.gl) {
        console.warn('WebGL renderer creation failed, falling back to CSS animation');
        return;
      }
      ...
      return () => {
        ...
        try {
          ...
          console.warn('Failed to lose WebGL context:', e);
        }
      };
    } catch (error) {
  ...
  // Return CSS fallback for unsupported browsers
  if (!isWebGLSupported()) {
    return <CSSFallback colorStops={colorStops} />;
  }
